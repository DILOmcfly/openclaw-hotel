...
export async function removeRelationship(
  ...
  await sql`
    ...
    WHERE agent_id = ${agentId} AND target_id = ${targetId} AND relationship_type = ${type}
  `;
}
...
export async function getRelationships(
  ...
  if (type) {
    return await sql<Relationship[]>`
      ...
      WHERE agent_id = ${agentId} AND relationship_type = ${type}
...
export async function isBlocked(
  ...
  const [result] = await sql`
    ...
    WHERE agent_id = ${agentId} AND target_id = ${targetId} AND relationship_type = 'blocked'
  `;
  return !!result;
}
