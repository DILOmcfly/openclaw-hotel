...
export async function createSeason(
  ...
  const [season] = await sql<Season[]>`
    INSERT INTO seasons (id, name, theme, start_date, end_date, weather_override, color_scheme)
    VALUES (${id}, ${name}, ${theme}, ${startDate}, ${endDate}, ${weatherOverride}, ${JSON.stringify(colorScheme)})
    RETURNING id, name, theme, start_date AS "startDate", end_date AS "endDate",
      is_active AS "isActive", weather_override AS "weatherOverride",
...
export async function getActiveSeason(sql: Sql): Promise<Season | null> {
  const [season] = await sql<Season[]>`
    SELECT id, name, theme, start_date AS "startDate", end_date AS "endDate",
      is_active AS "isActive", weather_override AS "weatherOverride",
      color_scheme AS "colorScheme", created_at AS "createdAt"
...
export async function activateSeason(seasonId: string, sql: Sql): Promise<void> {
  ...
  await sql`UPDATE seasons SET is_active = true WHERE id = ${seasonId}`;
}
...
export async function addSeasonalItem(
  ...
  const [item] = await sql<SeasonalItem[]>`
    INSERT INTO seasonal_items (id, season_id, item_type, name, description, rarity)
    VALUES (${id}, ${seasonId}, ${itemType}, ${name}, ${description}, ${rarity})
    RETURNING id, season_id AS "seasonId", item_type AS "itemType",
      name, description, rarity, available
  `;
  return item;
}

export async function getSeasonalItems(seasonId: string, sql: Sql): Promise<SeasonalItem[]> {
  return await sql<SeasonalItem[]>`
    SELECT id, season_id AS "seasonId", item_type AS "itemType",
      name, description, rarity, available
    FROM seasonal_items WHERE season_id = ${seasonId} ORDER BY name ASC
  `;
}

export async function isSeasonActive(seasonId: string, sql: Sql): Promise<boolean> {
  const season = await getSeasonById(seasonId, sql);
  return season ? season.isActive : false;
}
